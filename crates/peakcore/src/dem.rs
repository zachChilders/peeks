//! Copernicus GLO-30 tile indexing, bilinear sampling, and tile decoding.
//!
//! Deliberately no network/filesystem I/O here, matching this crate's policy (see the
//! module doc on [`crate`]): `peaklab` fetches tiles with blocking `reqwest` and a disk
//! cache, `src-tauri` fetches them with async `reqwest` and a Tauri-managed cache dir.
//! Both decode bytes with [`decode_tile`] and hand the grid to [`Dem::insert_tile`], so
//! the tile-index math and the DEFLATE/COG decoding itself — the parts that must agree
//! between platforms — live exactly once.
//!
//! Tiles are 1°×1°, 3600×3600 Float32 postings (1 arcsecond ≈ 30 m) on WGS84 lat/lon.
//! Postings sit on exact arcsecond multiples, so it is convenient to index them in a
//! single global grid:
//!
//! ```text
...
//!
//! Note this is a *surface* model: over a summit ice cap it reads meters above the
...
pub fn tile_name(lat0: i32, lon0: i32) -> String {
...
/// 1°×1° tile coordinates `(lat0, lon0)` covering a `radius_m` circle around
/// `(lat, lon)`, as `(south latitude, west longitude)` of each tile.
pub fn tiles_for_region(lat: f64, lon: f64, radius_m: f64) -> Vec<(i32, i32)> {
...
pub enum DecodeError {
    ...
    #[error("expected {TILE_DIM}x{TILE_DIM} tile, got {0}x{1}")]
    WrongDimensions(u32, u32),
    #[error("expected Float32 samples")]
    WrongSampleFormat,
}

/// Decode a Copernicus GLO-30 COG tile's raw bytes into a `TILE_DIM x TILE_DIM`
/// row-major grid.
pub fn decode_tile(bytes: &[u8]) -> Result<Vec<f32>, DecodeError> {
    let mut decoder = tiff::decoder::Decoder::new(Cursor::new(bytes))?;

    let (w, h) = decoder.dimensions()?;
    if w as i64 != TILE_DIM || h as i64 != TILE_DIM {
        return Err(DecodeError::WrongDimensions(w, h));
    }

    match decoder.read_image()? {
        tiff::decoder::DecodingResult::F32(v) => Ok(v),
        _ => Err(DecodeError::WrongSampleFormat),
    }
}

#[derive(Default)]
pub struct Dem {
    /// `None` means "confirmed absent upstream", so a caller does not retry the fetch.
...
impl Dem {
    ...
    /// Record a decoded tile (or `None` for a confirmed-absent one, e.g. all-ocean).
    pub fn insert_tile(&mut self, lat0: i32, lon0: i32, grid: Option<Vec<f32>>) {
        self.tiles.insert((lat0, lon0), grid);
    }

    pub fn has_tile(&self, lat0: i32, lon0: i32) -> bool {
        self.tiles.contains_key(&(lat0, lon0))
    }
    ...
    /// Build a `Dem` directly from in-memory tiles, bypassing decoding entirely.
    /// Test-only: lets visibility/geometry tests exercise real [`Dem`] sampling against
    /// hand-built synthetic terrain instead of real tile data.
    #[cfg(test)]
    pub fn from_tiles_for_test(tiles: Vec<((i32, i32), Vec<f32>)>) -> Self {
        let mut dem = Self::new();
        for (k, grid) in tiles {
            dem.insert_tile(k.0, k.1, Some(grid));
        }
        dem
    }
}
